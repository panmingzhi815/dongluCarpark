...
public class CarparkMainApp2 extends AbstractApp implements XinlutongResult {
	...
	private Logger LOGGER = LoggerFactory.getLogger(CarparkMainApp2.class);
	...
	@Inject
	private CarparkMainPresenter2 presenter;
	@Inject
	...
	// 是否中断收费操作
	private boolean stopCharge = false;
	...
	protected void createContents() {
		...
		trayItem.setToolTipText("停车场车牌识别监控界面");
		trayItem.setImage(JFaceUtil.getImage("carpark_24"));
		...
		GridData gd_text_total = new GridData(SWT.FILL, SWT.CENTER, true, false, 1, 1);
		gd_text_total.widthHint = 182;
		...
		GridData gd_text_hours = new GridData(SWT.FILL, SWT.CENTER, true, false, 1, 1);
		gd_text_hours.widthHint = 162;
		text_hours.setLayoutData(gd_text_hours);
		...
		button_1.setText("取消收费");
				...
				button.addSelectionListener(new SelectionAdapter() {
					...
					public void widgetSelected(SelectionEvent e) {
						...
						sashForm.setWeights(new int[]{1,random.nextInt(1)});
						sashForm_1.setWeights(new int[]{1,random.nextInt(1)});
					}
				});
		...
		button_2.addSelectionListener(new SelectionAdapter() {
			@Override
			public void widgetSelected(SelectionEvent e) {
				boolean confirm = commonui.confirm("退出提示", "确定退出停车场车牌识别程序？");
				if (confirm) {
					System.exit(0);
				}
			}
		});
		button_2.setLayoutData(new GridData(SWT.CENTER, SWT.CENTER, false, false, 2, 1));
		button_2.setText("退出");
		...
		sashForm_1 = new SashForm(composite, SWT.VERTICAL);
		...
		tabInFolder = new CTabFolder(composite_13, SWT.BORDER | SWT.FLAT);
		tabInFolder.setSize(264, 40);
		...
		tabOutFolder = new CTabFolder(composite_14, SWT.BORDER | SWT.FLAT);
		...
		addToolItemOfAdmin(toolBar, outToolBar);
		createDeviceTabItem();
	}

	private void addToolItemOfAdmin(ToolBar toolBar, ToolBar outToolBar) {
		if (!userType.equals("操作员")) {
