...
public class TempCarFreeView extends Composite implements View{
	private Presenter presenter;
	private Composite listComposite;
	private Text text_plateNO;

	public TempCarFreeView(Composite parent, int style) {
		super(parent, style);
		...
		button.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				try {
					getPresenter().search(text_plateNO.getText());
				} catch (Exception e1) {
					e1.printStackTrace();
				}
			}
	...
	@Override
	public TempCarFreePresenter getPresenter() {
		return (TempCarFreePresenter) presenter;
	}
