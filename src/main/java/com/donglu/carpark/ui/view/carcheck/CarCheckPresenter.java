...
public class CarCheckPresenter  extends AbstractPresenter{
	...
	public void search(String plateNo, Date start, Date end, String type, String status, String edit, String editPlateSize, double money, String operaName) {
		if ("全部".equals(type)) {
			type=null;
		}
		if ("全部".equals(status)) {
		...
		listPresenter.search(plateNo,start,end,type,status,edit==null?null:"已修改".equals(edit),StrUtil.isEmpty(editPlateSize)?null:Integer.valueOf(editPlateSize),money,operaName);
	}
	@Override
